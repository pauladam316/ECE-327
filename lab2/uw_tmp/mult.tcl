if { ![project_exists {mult}] } {
     project_new {mult}
} else {
     project_open {mult}
}
set_project_settings -cmp {mult}
if { ![project_settings_exist -sim mult] } {
        set_project_settings -sim mult
}
set_global_assignment -section_id {mult} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {VQM_FILE} {mult.vqm}
...
if { [file exists {mult_altera.prop}] } {
  source mult_altera.prop
}
set_global_assignment -name {FAMILY} {MAX 10 FPGA}
set_global_assignment -section_id {mult} -name {DEVICE} {10M08SAE144C8GES}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {mult_pnr_constraints.sdc}
